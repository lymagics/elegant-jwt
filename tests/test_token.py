...
def test_refuses_claims_once_its_own_clock_passes_expiration():
    assert_that(
        calling(
            JwtToken(
                "st.a.le",
                FakeSignature("st.a.le", {"sub": "3131", "exp": 2000}),
                FakeClock(2000),
            ).claims
        ),
        raises(Exception, "expired"),
...
def test_reads_back_claims_with_audience():
    assert_that(
        ...
        .token(
            AudienceSignature(
                Hs256("audience-secret-stretching-beyond-thirty-two-bytes"),
                "ledger-service",
            )
        )
        .claims()
